...
const HealthBadge = ({ health }) => {
  ...
  const label = health ? health.charAt(0).toUpperCase() + health.slice(1) : '—';
  return <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${s[health] || s.unknown}`}>{label}</span>;
};
