...
<body>
  <div class="no-print" style="background:#6366f1;color:white;padding:12px 20px;margin:-40px -40px 24px;display:flex;justify-content:space-between;align-items:center;">
    <span style="font-weight:600;">Print Preview — ${title}</span>
    <div>
      <button onclick="window.print()" style="background:white;color:#6366f1;border:none;padding:8px 16px;border-radius:6px;font-weight:600;cursor:pointer;margin-right:8px;">🖨️ Print / Save PDF</button>
      <button onclick="window.close()" style="background:rgba(255,255,255,0.2);color:white;border:none;padding:8px 16px;border-radius:6px;cursor:pointer;">✕ Close</button>
    </div>
  </div>
  <div class="header">
  ...
  ${content}
  <div class="footer">
    Generated by Tallowbourn Dashboard • ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}
  </div>
</body>
</html>`;
...
// ── Helper functions for building report content ──
...
// ── Report generators for each view ──
...
export const printInventory = ({ storeName, invHistory, savedCogs, appSettings }) => {
  ...
  let rows = products.map(p => {
    ...
    return tableRow([
      ...
      { value: daysLeft > 0 ? `${daysLeft}d` : '—', align: 'text-right' },
...
export const printSalesTax = ({ storeName, salesTaxConfig }) => {
  ...
  let rows = states.map(([code, config]) => {
    ...
    const regId = config.registrationId || '—';
...
export const printDailySummary = ({ storeName, allDaysData, dateRange }) => {
  ...
  printView({ title: 'Daily Sales Report', storeName, dateRange: dateRange || `${days[0]} – ${days[days.length - 1]}`, content });
};
